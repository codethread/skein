...
- **SPEC-002.C2:** Client JSON config lives at `config.json` inside the selected config-dir. It may contain only supported low-privilege keys such as `source` and `format`; malformed config, unsupported keys, or wrong value types fail non-zero.
- **SPEC-002.C3:** `source`, when required, must be an absolute path to the Atom source checkout root containing `deps.edn`. Relative paths, missing directories, and directories without `deps.edn` fail before launching Clojure.
