...
The CLI is the primary scripted interface for coding agents. It exposes a deliberately small strand surface: initialize storage, create strands, update strands, inspect strands, list strands, ask for ready work, and manage the local weaver runtime.

The public CLI is a thin Go executable named `strand`. It uses Cobra for command, subcommand, flag parsing, and help text; resolves the selected Skein world; sends JSON requests over the weaver's local Unix socket; formats human or JSON output; and never opens SQLite or evaluates rich query definitions locally.
...
```text
strand [--config-dir <dir>] [--format human|json] <command> [args]
```
...
- **SPEC-002.C6:** `add` creates a strand with generated id, `active`/`ephemeral` fields, timestamps, and string-valued CLI attributes. `--active` defaults to `true`; `--ephemeral` defaults to `false`.
- **SPEC-002.C7:** `update` patches title, active state, ephemeral retention, attributes, and strand edges for one existing strand.
- **SPEC-002.C8:** `--edge edge-type:to-id` creates or updates an outgoing edge from the updated strand to the target strand.
- **SPEC-002.C9:** `show`, `list`, and `ready` return strand rows with normalized `attributes` in JSON output and fields `active`, `ephemeral`, and `inactive_at`; they do not emit `status` or `final_at`.
...
- **SPEC-002.C12:** `--where`, `--status`, and `--format edn` are not part of the public Go CLI. Rich EDN query authoring belongs in trusted weaver config and REPL workflows.
