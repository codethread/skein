...
- **SPEC-002.C20:** `weaver status` asks `mill` for selected workspace status and reports health, friendly name, selected config/data paths, weaver storage metadata (`database_kind`, `database_label`, and `database_path`), pid, weaver identity, socket endpoint, and nREPL endpoint when running. `database_path` is a canonical path string for `sqlite-file` storage and explicitly `null` for `sqlite-memory`; the field is not omitted. `weaver stop` asks `mill` to stop only the selected workspace weaver child and clean that workspace's runtime metadata/socket.
...
- **SPEC-002.C20b:** No public CLI command or flag selects weaver storage kind, and no library-testing commands exist. CLI metadata/status validation compares `database_kind` and `database_label` for all storage kinds and `database_path` only for `sqlite-file`, failing loudly on malformed or inconsistent storage metadata instead of assuming file storage. Mill supervises file-backed workspace weavers only and rejects other storage kinds in mill-managed weaver metadata.
