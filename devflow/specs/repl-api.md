...
- **SPEC-003.C59c:** Handler symbols resolve in the weaver JVM/runtime classloader, matching event/view/pattern conventions, and are persisted as a symbol/string representation that survives restart; closures and anonymous functions are rejected. Handlers receive one context map (SPEC-004.C103), run through the weaver's serialized async lane, have their return value ignored, and exceptions do not crash the scheduler.
