...
- **SPEC-003.C19a:** Converted `skein.api.*.alpha` modules follow the v1 form contract. The `alpha` namespace holds only the public promised vars, ordered public-first — the surface at the top of the file, lower-level detail below, with `declare` used where definition order would otherwise fight reading order. Implementation plumbing lives in the sibling `skein.api.<module>.internal` namespace, which the exclusionary rule of SPEC-005.P1 places outside the contract; a module with no plumbing ships no internal namespace. Source lines in a converted module's files stay within 96 columns, with docstrings and comments hard-wrapped in source. `quality.conventions-check` enforces the mechanical parts and its `pending` set names the modules not yet converted; the worked example is `devflow/feat/g1men-v1-api-format/`.
