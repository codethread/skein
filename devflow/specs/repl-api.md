...
- **SPEC-003.C19a:** Converted `skein.api.*.alpha` modules follow the v1 form contract. The `alpha` namespace holds only contract-bearing top-level forms — public promised vars, each carrying a docstring, and intentionally public spec registrations whose qualified keys are part of the promise — ordered public-first: the surface at the top of the file, lower-level detail below, with `declare` used where definition order would otherwise fight reading order. Implementation plumbing lives in the sibling `skein.api.<module>.internal` namespace, which the exclusionary rule of SPEC-005.P1 places outside the contract; a module with no plumbing ships no internal namespace. Public vars stay defined in the alpha namespace, never re-exported from internal; an internal namespace never requires an alpha namespace, and among `src` namespaces only a module's own alpha requires its internal. Every public fn's interface is identified from alpha: its named argument/return specs or `s/fdef` register there, keeping their qualified keys on the promised namespace; reusable sub-specs may live in internal when they are plumbing rather than promise. Source lines in a converted module's files stay within 96 columns, with docstrings and comments hard-wrapped in source. `quality.api-form` (run by `quality.conventions-check`) enforces the mechanical parts and its `pending` set names the modules not yet converted; the worked example is `devflow/feat/g1men-v1-api-format/`.
