...
- `skein.api.events.alpha` exposes `(register! runtime key types fn-sym)`, `(register! runtime key types fn-sym metadata)`, `(unregister! runtime key)`, `(handlers runtime)`, `(recent-failures runtime)`, and `(enqueue! runtime event)`. `enqueue!` submits an event map to the runtime event system for asynchronous dispatch, validating the `:event/type`, `:event/id`, `:event/at`, and `:event/source` envelope.
