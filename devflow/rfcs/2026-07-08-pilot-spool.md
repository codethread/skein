...
- **RFC-020.P1.1:** `:self` steps and checkpoints progress only while a driver
...
- **RFC-020.P1.2:** No contract spawns a driver on attention. `workflow/await!`
...
- **RFC-020.P1.3:** The merge lock serializes local main mutation but defines
...
evidence that goes stale after fix commits. RFC-020.P6 handles each.
...
- **RFC-020.G1:** Launch a whole feature once and have it run intake through
  land with human input only at the declared human-only points and at loud
  escalations.
- **RFC-020.G2:** Coordinate through ephemeral seats: the system spawns a
  ...
  exits.
- **RFC-020.G3:** Run a merge train on the merge sentinel: signed-off pilot
  runs with green CI land automatically, one at a time, and lock release
  admits the next queued run.
- **RFC-020.G4:** Leave a durable note on every automatic transition naming
  the HEAD sha, the run ids involved, and the evidence, so cold resume and
  later review keep working.
- **RFC-020.G5:** Keep TEN-003/TEN-004 discipline: every loop bounded, every
...
- **RFC-020.G6:** Give each of the four observed anomaly classes an explicit,
  bounded recovery or escalation path.

## RFC-020.P3 Non-goals

- **RFC-020.NG1:** Do not auto-decide human checkpoints. `workflow/hitl`
  stays load-bearing; pilot never downgrades a human-only point to an agent
  checkpoint.
- **RFC-020.NG2:** Do not add engine primitives in v1: no scheduler beyond
...
- **RFC-020.NG3:** Do not build replay or audit machinery. Resumability only,
  per PHILOSOPHY: notes exist for the next reader, and the code wins over the
  record.
- **RFC-020.NG4:** Do not design a multi-lane or cross-repo train. One local
  main, one lock, one queue.
- **RFC-020.NG5:** Do not retire hands-on coordination. Pilot is a per-feature
  launch choice; resident driving of devflow and land keeps working unchanged.
- **RFC-020.NG6:** Do not change the worker contract or expand worker
...
- **RFC-020.O1:** Chime rules over the existing devflow and land runs: rules
...
- **RFC-020.O2:** A kanban-attribute state machine: a cron job scans card
...
- **RFC-020.O3:** A pilot workflow family: one run-id from intake to land,
...
- **RFC-020.O4:** Resident coordinator (status quo): a live session
...
- **RFC-020.O5:** Ephemeral seats: a seat is spawned per decision point,
...
- **RFC-020.O6:** Hybrid: resident through design, ephemeral after plan
...
- **RFC-020.O7:** Chime rule side effects spawn seats. Event-driven with no
...
- **RFC-020.O8:** A `.skein` dispatcher: a cron job sweeps pilot runs for
...
- **RFC-020.O9:** A new reference-spool adapter (a treadle sibling) that
...
- **RFC-020.O10:** Change the shared land family so sign-off auto-approves
...
- **RFC-020.O11:** Put the policy in seat prompts and let the seat judge.
  ...
  silently improvises.
- **RFC-020.O12:** Policy as `.skein` data: machine-checkable criteria the
...
## RFC-020.P5 Recommendation

- **RFC-020.REC1:** Choose **O3 + O5 + O8 + O12**: a pilot workflow family in
...
- **RFC-020.REC2:** Run shape: one run-id from intake to land. Reuse the
...
- **RFC-020.REC3:** Checkpoint ownership under pilot: brief capture and
...
- **RFC-020.REC4:** The dispatcher is a cron job over pilot run roots plus
...
- **RFC-020.REC5:** Seat contract and lease: the dispatcher stamps
  ...
  escalates.
- **RFC-020.REC6:** Review cycle as routing: after AFK completion the run
  ...
  history.
- **RFC-020.REC7:** Merge train on the merge sentinel:
...
- **RFC-020.REC8:** Escalation endpoints are chime rules, kept strictly as
...
## RFC-020.P6 Anomaly classes

- **RFC-020.A1 Stale run-id squat.** A stale `treadle/run` or `pilot/seat`
...
- **RFC-020.A2 Hung worker, PID-only kills.** Treadle has no wall-clock
...
- **RFC-020.A3 Partial handover, cold resume.** Every seat decision note is
...
- **RFC-020.A4 Review-evidence staleness.** Findings are live only for the
...
## RFC-020.P7 Placement: policy, spool, engine
...
## RFC-020.P8 Acceptance criteria

- **RFC-020.AC1:** One real, small feature runs intake through land on this
...
- **RFC-020.AC2:** With auto-land granted, two pilot runs queue on the
  sentinel and the second lands with no human merge command after the first
  releases the lock.
- **RFC-020.AC3:** Each of the four anomaly classes is induced in a
...
- **RFC-020.AC4:** Provenance audit: every checkpoint close in the pilot run
  carries `:by`; no `workflow/hitl` checkpoint was closed by a seat.
- **RFC-020.AC5:** Evidence audit: every automatic transition note names the
  HEAD sha and run ids; a cold agent given only the run record states the
  feature's current state correctly.
- **RFC-020.AC6:** Escalation drill: exhausting a review-round bound and a
  hang budget each produce a chime notification and a durable escalation
  note, with `chime/failures` empty.

## RFC-020.P9 Open questions

- **RFC-020.Q1:** The devflow reuse seam: does `skein.spools.devflow` expose
...
- **RFC-020.Q2:** Seat batching: one attention state per seat is the v1 rule;
  ...
  grain.
- **RFC-020.Q3:** Hang budgets: measured from spawn or from last observed
  activity, and per-harness defaults — shuttle records enough to choose, but
  the right numbers need observation.
- **RFC-020.Q4:** Whether task-breakdown acceptance should stay human for the
...
- **RFC-020.Q5:** Train ordering beyond FIFO, and cross-feature conflicts:
...
- **RFC-020.Q6:** Promotion trigger: what evidence (how many landed features,
