...
`001`; `PROP-Dcr-001` is the first ID under this prefix.

## PROP-Dcr-001.P1 Problem
...
## PROP-Dcr-001.P2 Goals

- **PROP-Dcr-001.G1:** An op declaration may carry a data-first
...
- **PROP-Dcr-001.G2:** Shipped op tests pass their captured results
...
- **PROP-Dcr-001.G3:** Runtime validation occurs at genuine
...
- **PROP-Dcr-001.G4:** A blessed spool-authoring constructor returns
  exactly the canonical entity projection
  `{:id :title :state :attributes}`, so ordinary entity payloads share one
  source shape.
- **PROP-Dcr-001.G5:** The declaration, help projection, tests, and
...
## PROP-Dcr-001.P3 Non-goals

- **PROP-Dcr-001.NG1:** No live-serving-path output validation.
...
- **PROP-Dcr-001.NG2:** No runtime output validation outside real
  spool-to-spool consumption seams. Human- or agent-read CLI JSON is checked by
  tests, not on every request.
- **PROP-Dcr-001.NG3:** No blanket `clojure.spec` per op and no
...
- **PROP-Dcr-001.NG6:** No implementation sequence, task slicing,
  or landing strategy. Those belong to the plan.

## PROP-Dcr-001.P4 Existing seams

### PROP-Dcr-001.P4.1 Declaration and help
...
### PROP-Dcr-001.P4.2 Generated docs
...
### PROP-Dcr-001.P4.3 Test capture and CI failure
...
### PROP-Dcr-001.P4.4 Spool-to-spool consumption
...
### PROP-Dcr-001.P4.5 Entity projection adoption
...
## PROP-Dcr-001.P5 Proposed scope

- **PROP-Dcr-001.S1:** Extend the op registry metadata contract with
  `:returns`, including loud declaration validation and preservation on
  register and replace.
- **PROP-Dcr-001.S2:** Extend full op help detail with a JSON-safe
  rendering of the declared return shape. Help listing stays summary-only.
- **PROP-Dcr-001.S3:** Extend workspace `defop` and direct
  registrations through the existing metadata route; do not add a parallel op
  registry or documentation store.
- **PROP-Dcr-001.S4:** Add one blessed author-side helper that checks
...
- **PROP-Dcr-001.S5:** Add runtime shape checking only where a
...
- **PROP-Dcr-001.S6:** Add the four-field entity constructor to the
...
- **PROP-Dcr-001.S7:** Update owning root specs and public API
...
## PROP-Dcr-001.P6 Return-shape language

- **PROP-Dcr-001.Q1:** What is the smallest useful `:returns`
  ...
  outside the minimum.
- **PROP-Dcr-001.Q2:** Does the language live in
...
- **PROP-Dcr-001.Q3:** Do batteries `show`/`list` deliberately
...
## PROP-Dcr-001.P7 Root-spec obligations

- **PROP-Dcr-001.RS1 — `devflow/specs/cli.md`:** amend
...
- **PROP-Dcr-001.RS2 — `devflow/specs/daemon-runtime.md`:** amend
...
- **PROP-Dcr-001.RS3 — `devflow/specs/repl-api.md`:** extend
...
- **PROP-Dcr-001.RS4 — `devflow/specs/alpha-surface.md`:** amend
