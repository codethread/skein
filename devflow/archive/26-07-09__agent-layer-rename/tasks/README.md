...
- **PH2 (fan-out after PH1):** `006`, `007`, `008` parallelize (disjoint source families:
  agent-run / subagent-gate / delegation). `009` (workflow-core reads) **serializes after
  `006`+`007`** because its projections and focused tests read the renamed run/gate markers those
  two tasks produce — landing it earlier would assert against a half-renamed marker vocabulary.
  `010` (event kws) likewise **serializes after `006`+`007`** because it edits their two files —
  never two mutators in one file (`PLAN-Alr-001.TC2`).
...
- **PH4 (fan-out after PH2):** `015` (.skein config), `016` (agent-dash), `017` (bench/chime)
  parallelize on disjoint files. `017` needs `006`+`007`+`008` — chime notifies on `review/*`,
  `panel/*`, and `note/*`, so it must wait on the delegation-side split (`008`) too.
...
The `make test-warm` target ships with the tiered-test-validation queue that Task `001` rebases
onto (it does not exist on this branch's pre-rebase base). Every `make test-warm` reference in these
tasks therefore assumes the PH0 rebase has landed; because `001` is the root of the serial spine and
every task blocks on it transitively, this holds by construction. `001` verifies the target resolves
...
| 009 | PH2d workflow/outcome-notes + workflow-core | build | 006, 007 |
...
| 017 | PH4c bench/chime consumer reconcile | build | 006, 007, 008 |
...
```json
{
  ...
  "tasks": [
    ...
    {"key": "009", "title": "PH2d workflow/outcome-notes + workflow-core", "harness": "build", "depends_on": ["006", "007"], "body": "See tasks/009-attr-workflow-outcome-notes.md (TASK-Alr-009)."},
    ...
    {"key": "017", "title": "PH4c bench/chime consumer reconcile", "harness": "build", "depends_on": ["006", "007", "008"], "body": "See tasks/017-bench-chime-consumer-reconcile.md (TASK-Alr-017)."},
