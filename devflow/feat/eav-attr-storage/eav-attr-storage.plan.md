...
| EAS-PLAN-001.AA3 | `src/skein/core/specs.clj` | **Remove** `::indexed-attr-key`. Keep `::attributes` (the map-shape contract on writes) and `::omitted-attribute-descriptor` (the L1 descriptor) unchanged. |
| EAS-PLAN-001.AA4 | `src/skein/api/weaver/alpha.clj` | Add trusted `archive!`/`unarchive!`/`migrate-attribute-storage!` ops with the arg/result/error contracts in `EAS-DELTA-003.CC4`; keep in-process reads full-fidelity over the assembled map (archived included). **Remove** the declare/list indexed-attr-key ops. No lean transform in `normalize-row`. |
| EAS-PLAN-001.AA5 | `spools/src/skein/spools/batteries.clj` | No behavior change: `list-op`/`ready-op`/named-query lean projection and full `show-op` operate over the assembled map exactly as today. Confirm-only. |
...
- **EAS-PLAN-001.CM1:** This is an incompatible core-schema change (the `strands.attributes` column is dropped). `ensure-current-schema!` fails loud on an old world; there is no silent migration (TEN-000, TEN-003). New worlds get the row schema natively.
- **EAS-PLAN-001.CM2:** Cutover is one explicit trusted migrate op, `migrate-attribute-storage!`, run per existing world, including the canonical coordination world, which carries real data (~2k strands, 4.67MB attribute bytes). The op reads each `strands.attributes` document, writes one row per key (all `archived = 0`), verifies row/document parity, fails loudly on any mismatch, and is safe to re-run on an already-migrated world (`EAS-DELTA-003.CC4`/`D2`).
...
- **EAS-PLAN-001.CM5:** `archive!`/`unarchive!`/`migrate-attribute-storage!` are trusted Clojure surface only — no socket op, no `strand` command (TEN-006). Core ships no archive policy. Bad strand ids, malformed or empty key sets, missing keys, partial archive writes, malformed source documents, mixed schemas, and migrate parity mismatches fail loudly with diagnostic ex-data (`EAS-DELTA-003.CC4`).
...
Outcome: `archive!`/`unarchive!` flip the `archived` flag per strand and per explicit key set; the `WHERE archived = 0` partial index and an `archived = 0` predicate keep archived rows out of assembly-for-list and every query path; full point reads still include them. Tests cover per-strand and explicit-key-set archive/unarchive, hot-path exclusion, full-read inclusion, and fail-loud behavior for bad strand ids, malformed/empty key sets, missing keys, and partial writes.
...
Outcome: `migrate-attribute-storage!` converts document worlds to rows with parity verification and re-run safety; the `indexed_attr_keys` table, its trusted ops, and the L0b tests are removed. Tests cover document→rows fidelity, idempotency on an already-migrated world, and loud failure on a malformed source document, mixed old/new schema, and any post-write parity mismatch.
...
- **EAS-PLAN-001.R4 (migrate on the canonical world):** the canonical coordination world carries live data; a lossy or non-idempotent migrate would be a real-data incident. Mitigation: `PH4` verifies row/document parity, is re-run-safe, and is exercised in tests before it is ever pointed at the canonical world; run it against a disposable copy first.
...
- **EAS-PLAN-001.TC3:** Split reads by caller unchanged: lean at the CLI/agent op boundary, full for `show` and every trusted in-process `api/*` read (archived values included). Never lean-project in `normalize-row`.
...
- **EAS-PLAN-001.TC5:** Archive is a flag, not a table or a second concept. Ship `archive!`/`unarchive!` as trusted primitives only; ship no archive policy and no CLI command. Preserve the fail-loud bad-input contract from `EAS-DELTA-003.CC4`.
- **EAS-PLAN-001.TC6:** Cutover is explicit and once: fail loud on the document schema, migrate via `migrate-attribute-storage!` with parity verification. Never auto-migrate on open (TEN-000, TEN-003). A parity mismatch is a hard failure, not a warning.
- **EAS-PLAN-001.TC7:** The benchmark gate (`P7`) is merge-blocking and its numbers are exact. Serialize timed runs behind the benchmark lock; report a missed target, never soften it.
