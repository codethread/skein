...
- **EAS-PLAN-001.TC6:** Cutover was explicit and once: fail loud on the document schema, migrate via `migrate-attribute-storage!` with parity verification. Never auto-migrate on open (TEN-000, TEN-003). The one-shot migrate op shipped for that cutover window and was then struck from main.
