...
| EAS-PLAN-001.AA3 | `src/skein/core/specs.clj` | **Remove** `::indexed-attr-key`. Keep `::attributes` (the map-shape contract on writes) and `::omitted-attribute-descriptor` (the L1 descriptor) unchanged. Add spec-backed contracts for the new trusted seam shapes: `::attribute-key-set`, `::attribute-archive-result`, `::attribute-storage-migration-result`, plus fdefs or equivalent call-site conformance for `archive!`/`unarchive!`/`migrate-attribute-storage!`. |
| EAS-PLAN-001.AA4 | `src/skein/api/weaver/alpha.clj` | Add trusted `archive!`/`unarchive!`/`migrate-attribute-storage!` ops with the arg/result/error contracts in `EAS-DELTA-003.CC4`; keep point/trusted lookup reads full-fidelity over the assembled map (archived included) while hot list/query reads return non-archived maps only. **Remove** the declare/list indexed-attr-key ops. No lean transform in `normalize-row`. |
