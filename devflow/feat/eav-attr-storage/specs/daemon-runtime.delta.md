...
- **EAS-DELTA-003.CC4:** The weaver API gains three trusted semantic operations, all trusted Clojure config/REPL surface with **no** public JSON socket operation and **no** `strand` command (same tier as SPEC-004.C16/C16a's trusted registry helpers, TEN-006):
  - `(archive! runtime strand-id)` archives all attributes for one strand; `(archive! runtime strand-id keys)` archives only the explicit key set. `(unarchive! runtime strand-id)` and `(unarchive! runtime strand-id keys)` are symmetric. `strand-id` is a non-blank string id; `keys` is a non-empty collection of attribute keys. Return shape is a map at least `{:strand-id <id> :keys [<key> ...] :archived? true|false :changed <n>}` with keys returned in lexicographic order. Unknown strand ids, malformed ids, malformed key collections, empty key sets, keys not present on the strand, and archive/unarchive writes that affect fewer rows than requested fail loudly with ex-data carrying `:strand-id`, `:keys`, and a stable reason keyword. They mutate the `archived` flag only; they never change a value or the map contract. Archive **policy** stays userland (`PROP-EavAttrStorage-001.NG4`).
  - `(migrate-attribute-storage! runtime)` converts an existing world's `strands.attributes` documents into `attributes` rows, so existing worlds — including the canonical coordination world — cut over explicitly and once (`EAS-DELTA-001.CC8`). Return shape is a map at least `{:status :migrated|:already-current :strands <n> :attributes <n> :verified? true}`. It fails loudly on a malformed source document, a partially migrated mixed schema, any post-write row/document parity mismatch, or any storage schema that is neither the old document layout nor the current row layout, with ex-data carrying enough counts/ids to diagnose the failure. Re-running on an already-migrated world returns `:already-current`.
...
- **EAS-DELTA-003.CC7:** Weaver API return-value normalization (SPEC-004.C17) is unchanged on the trusted in-process path: `skein.api.weaver.alpha` reads (`show`, `list`, `ready`, `strands-by-ids`, `subgraph`, and the query-execution ops as consumed in process) decode the assembled attribute rows to full-fidelity Clojure maps, including archived values (`EAS-DELTA-001.CC4`). The lean tier stays a CLI/agent read-surface transform layered above these ops, so the trusted path a spool calls never applies it.
...
- **EAS-DELTA-003.CC9:** Mutation event and view payloads (SPEC-004.P10a, SPEC-004.C71–C74; SPEC-004.C56–C59) that embed a strand carry the **same assembled attribute map** they carry today — full-fidelity for in-process consumers, the L1 lean projection for read-surface consumers. Their shape does not change; the storage change is invisible to them (`PROP-EavAttrStorage-001.D4`).
...
- **Rationale:** They mutate storage-tier state that must not be reachable from the low-privilege CLI worker surface (TEN-006), mirroring the tier at which `declare-acyclic-relation!` and the removed indexed-key ops lived. What to archive and when (e.g. a userland gleaner over closed strands, which hold ~99% of payload bytes) is a userland decision, not a core one (TEN-004, `PROP-EavAttrStorage-001.NG4`).
- **Rejected:** A public `strand archive` command; a shipped archive policy or scheduled sweeper in core.
...
- **EAS-DELTA-003.Q1:** The exact namespace placement and internal helper names for `archive!`, `unarchive!`, and `migrate-attribute-storage!` are implementation mechanics. Their trusted tier, semantic args, return shapes, and fail-loud cases are specified in `EAS-DELTA-003.CC4`; no contract-scope questions remain.
