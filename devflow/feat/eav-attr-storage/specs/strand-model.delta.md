...
- **EAS-DELTA-001.CC3:** The two read tiers (SPEC-001.P4) are unchanged. The full tier returns every visible attribute value verbatim; the lean tier returns the `::specs/omitted-attribute-descriptor` for values above the fixed 1024-byte floor. The L1 lean tier stays a wire-level transform applied over the **assembled** attribute map at the CLI/agent read surface — the byte floor, the omission descriptor, the caller split (lean `list`/`ready`/query; full `show` and trusted point/lookup reads), and the `attr-get` fail-loud guard operate over the assembled map exactly as they did over the document. Archive visibility is orthogonal to leanness and is fixed by `EAS-DELTA-001.CC4`.
