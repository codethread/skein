...
- **EAS-DELTA-001.CC3:** The two read tiers (SPEC-001.P4) are unchanged. The full tier returns every visible attribute value verbatim; the lean tier returns the `::specs/omitted-attribute-descriptor` for values above the fixed 1024-byte floor. The L1 lean tier operates over the **assembled** attribute map at the CLI/agent read surface — the byte floor, the omission descriptor, the caller split (lean `list`/`ready`/query; full `show` and trusted point/lookup reads), and the `attr-get` fail-loud guard operate over the assembled map exactly as they did over the document. The ready path may apply this projection in storage-backed SQL so oversized values are not fetched only to be omitted. Archive visibility is orthogonal to leanness and is fixed by `EAS-DELTA-001.CC4`.
- **EAS-DELTA-001.CC4:** Attribute rows carry an `archived` flag. Full-fidelity point/lookup reads (`show`, trusted reads by id, and subgraph row loads) transparently include archived values — a correct-but-slower cold read is acceptable (TEN-003 forbids *wrong*, not *slow*). Hot list-style paths (`list`, `ready`, query-backed listing, and every query predicate) structurally exclude archived rows by contract (`EAS-DELTA-001.CC12`), both for predicate matching and returned maps. The query author sees no declaration or threshold, but archive policy authors must understand that archiving a key removes it from hot-path membership tests such as `:exists`/`:missing`. Archived keys are the single explicit exception to CC1's byte-identity: they are present in full point reads and absent from hot query paths.
