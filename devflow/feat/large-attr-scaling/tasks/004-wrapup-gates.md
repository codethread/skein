...
- **TASK-LargeAttrScaling-004.MI1:** Add a `## Re-running the harness` section to `assessment-report.md` documenting the Task 2 full-scale invocation for re-runs — the bench-locked `mktemp -d` `${ws:?}` command, the env gate `SKEIN_LARGE_ATTR_BENCH_FULL=1`, the seed-profile defaults (`--seed 1337 --n 250000`), and where the raw EDN lands under `results/`. Dev-facing only — this stays a dev/test tool, not a shipped command or agent surface (`NG3`).
