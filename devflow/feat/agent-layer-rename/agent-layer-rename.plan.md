...
- **PLAN-Alr-001.CM2:** Durable contract changes are staged in the three feature deltas (`SPEC-Alr-001`/`-002`/`-003`) and promoted to root specs only after validation (PH5). Both edits per delta are token swaps plus one additive statement each; no behavior contract moves.
...
Gate: `make docs-check` at zero findings (with widened pathspec); `mkdocs build` (or docs-check dry run) clean; `make api-docs` regen shows only the expected renamed outfiles in `git status --short`; grep of docs tree clean of `shuttle`/`treadle`/`reed`/`agents` tokens outside archive.
...
Outcome: `SPEC-Alr-001`/`-002`/`-003` edits are applied to the three root specs (the two token-swap examples plus the three additive contract statements — the institutionalized naming rule, the frozen trained-vocabulary surface, the off-lane example rename), assigning the next free `SPEC-005.Cn` at edit time; the three deltas are marked Merged.
...
- **PLAN-Alr-001.V4:** The `PROP-Alr-001.DW1` whole-tree grep is the done-when proof: each old token (`shuttle/`, `treadle/`, `workflow/notes`, `skein.spools.shuttle|agents|treadle|reed`) returns only alias locals and `devflow/archive/*`.
...
- **PLAN-Alr-001.R1:** Rebase-over-in-flight-landings risk. Implementation is blocked (`PROP-Alr-001.R1`) behind the tiered-test-validation queue (in flight now — it lands `scripts/test-warm` and the shard tiers the focused gates depend on) and `vk8aa`'s shard-tier cleanup (deletes the redundant `shuttle_test` sync deftest and reworks shard ordering — exactly the suite files this feature renames). Mitigation: **PH0 is a hard gate** — rebase over both and reconcile before any mutating phase; do not rename suite files that `vk8aa` is still editing.
- **PLAN-Alr-001.R2:** Sibling-file overlap. `workflow-shell-gates` builds the reed/`:shell` executor in the file this feature moves to `executors/shell.clj`; `harness-alias-registries` (landed at `af6a1d0`) split the shuttle harness defs. A rename over a live edit is a rebase conflict, not just a naming clash (`PROP-Alr-001.R2`). Mitigation: land after the siblings or coordinate ordering with their owners; the sweep is a whole-tree cutover and does not interleave cleanly.
- **PLAN-Alr-001.R3:** Adjacent carded work touches the renamed surface and must be sequenced, not folded in. `2ms8c` (run-usage records) and `xwhe7` (treadle gate-advance enforcement) both mutate `shuttle/*`/`treadle/*` behavior; `cmqpb` (shuttle-dash synthesis-arrow fix) edits the very `scripts/shuttle-dash` this feature renames. Sequencing notes are already on their cards. Mitigation: none of these land inside this feature's window; each is behavior (`kaans` F2+) or waits for the dir rename. Any behavior discovered mid-sweep is carded, never folded (`PROP-Alr-001.NG1`).
