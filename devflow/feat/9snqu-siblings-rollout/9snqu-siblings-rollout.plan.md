...
- **PLAN-Sbl-001.A5 (skein-src):** Convert `test/skein/config_ops_test.clj:155` to the literal v8-compatible image declaration (require `ct.spools.kanban`, then `runtime/module!` with the `:ns`/`:load :image`/`:contribute`/`:reconcile` literal), verified green against the pinned v8. Record notebook.spool/notes clean-verification on the card. Land the skein-src branch (devflow docs + this conversion) via `strand land`.
