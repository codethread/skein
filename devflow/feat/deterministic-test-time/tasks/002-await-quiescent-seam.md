...
- **TASK-Dtt-002.MI4:** Add direct coverage of the not-report-early guard: a handler mid-dispatch
  with an empty queue must not let `await-quiescent!` return, and a timeout must throw.

## TASK-Dtt-002.P3 Done when

- **TASK-Dtt-002.DW1:** `clojure -M:test skein.weaver-test` is green (weaver-test exercises the
  event lane), plus the new `await-quiescent!` coverage namespace named in this task's diff.
- **TASK-Dtt-002.DW2:** `make fmt-check lint reflect-check` pass for the touched namespaces.
- **TASK-Dtt-002.DW3:** `test/skein/test_runner.clj` is untouched.
