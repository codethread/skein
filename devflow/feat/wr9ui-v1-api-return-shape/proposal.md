...
- **PROP-Vrs-001.A2:** All plumbing moves to `skein.api.return-shape.internal`; `alpha.clj` keeps the three documented fns delegating thinly, public-first. The published ex-data key stays literally `:skein.api.return-shape.alpha/error` in internal — the qualified-key hazard the counsel named; consumers and error shapes see zero change.
