...
`skein.api.*.alpha` namespace, no reference-spool promotion, no tier boundary moves. This file records the disposition
explicitly, following F4's real `SPEC-Vr-002` (which *added* `vocab` to the blessed enumeration) only in form — F-Ru's
disposition is no-change, like F3's `SPEC-Np` dispositions for untouched tiers.
...
- None. `SPEC-005.C4` (line 14) already places the `spools/agent-run` root and `spools/delegation` in userland — "their
  ...
  subcommand in `spools/delegation.md` and the `strand agent about` manual, each within its own userland doc cadence, so
