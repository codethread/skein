...
`SPEC-002` already describes; it moves no `SPEC-002` contract text. This file exists so the F-Ru delta set carries an
explicit disposition for every root spec, mirroring F4's `SPEC-Vr-003`.
...
- None. `strand agent spend` is a `:subcommands` entry on the `agent` op registered by `skein.spools.delegation`; its
  ...
  in `spools/delegation.md` and the `strand agent about` manual (`SPEC-005.C4`; `PROP-Ru-001.C8`). The dispatcher parses
...
- **SPEC-Ru-003.F1:** None. No dispatcher flag, entrypoint, selection-precedence, payload-slot, or mill-command
  ...
  is an op result shape documented in `spools/delegation.md`, not the byte-faithful transport contract `SPEC-002.C4`
