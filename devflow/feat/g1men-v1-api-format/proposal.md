...
- **PROP-Vaf-001.G1:** One file is the promise. A converted module's `skein.api.<mod>.alpha` namespace holds only public promised vars, ordered public-first: the surface at the top, lower-level detail below, `declare` where definition order fights reading order. Plumbing moves to a sibling `skein.api.<mod>.internal` namespace, which the exclusionary rule in SPEC-005.P1 already places outside the contract — no tier-membership change. A module with no plumbing ships no internal file.
- **PROP-Vaf-001.G2:** Wrapped prose is a gate, not a request. `quality.conventions-check` grows a converted-module check: no private vars in `alpha.clj`, and no source line past column 96 in the module's files. A `pending` set names the sixteen unconverted modules; each sibling card deletes its entry, and a pending module that has become fully conformant is itself a finding, so the set cannot rot.
...
## PROP-Vaf-001.P5 Decision links
