...
3. Classify every top-level form: contract-bearing (public promised var, intentionally public spec registration — these stay in `alpha.clj`, spec keys unchanged) or plumbing (moves to `skein.api.<module>.internal`). Reorder `alpha.clj` public-first (`declare` where needed). Do not create an internal file for a module with no plumbing. Hazards the counsel named: tests or `with-redefs` reaching into privates you are moving; Var identity and metadata that callers alias; never re-export an internal var from alpha — public vars stay defined in alpha. The dependency rules are gated: internal never requires alpha, and only the module's own alpha (or a test) requires its internal.
