...
- **ERF-TASK-009.MI1:** Update `skein.db/apply-batch!` and related validation so generic batch strand create/update entries use `:state` instead of legacy boolean lifecycle query field, accepting only `"active"` and `"closed"` through generic batch payloads.
...
- **ERF-TASK-009.MI5:** Update batch graph upsert tests and smoke assertions that mention legacy boolean lifecycle query field, old `active` JSON output, closed relation allowlist, or whole-graph cycle invariant so they prove the new state/relation behavior instead.
...
- **ERF-TASK-009.DW3:** Grep checks over current code/docs, excluding `devflow/archive`, show no stale batch graph upsert contract references to legacy boolean lifecycle query field, old JSON `active`, allowed edge-type allowlists, or whole-graph acyclicity.
