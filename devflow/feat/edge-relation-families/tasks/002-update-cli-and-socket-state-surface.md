...
- **ERF-TASK-002.MI3:** In `cli/internal/command/command.go`, replace `--active` with `--state` for `add`, `update`, and `list`. `add`/`update` accept `active|closed`; `list` accepts `active|closed|replaced`.
- **ERF-TASK-002.MI4:** Remove or replace the Go `boolFlag` lifecycle path with explicit state validation. Do not keep `--active` as a hidden alias.
...
- **ERF-TASK-002.DW3:** `rg -- '--active' cli src test` returns no public CLI/socket compatibility paths.
