...
**Owned files (disjoint from Tasks 1 and 3):**

- `src/skein/api/batch/alpha.clj`
- `test/skein/weaver_test.clj`
- `test/skein/api/batch/alpha_test.clj`
- `docs/api/batch.api.md` (generated by `make api-docs`)
...
## TASK-Xer-002.P6 Commit discipline

- **TASK-Xer-002.CD1:** Make one atomic conventional commit, `docs(api): document batch edge transitions`, containing only owned files. Do not amend, push, or include generated runtime artifacts.
- **TASK-Xer-002.CD2:** Before committing, `git status --short` must contain only expected owned-file changes; after committing, the worktree must be clean.
