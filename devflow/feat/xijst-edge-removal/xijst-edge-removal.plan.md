...
- **PLAN-Xer-001.A5 — public normalization, hook, and event:** `skein.api.batch.alpha/apply!` already applies `access/normalize` to the complete db result before it builds the pre-commit `:batch/edge-ops` and post-commit `:batch/edges` channels. The normalized transition shape therefore reaches the returned result, pre-commit gate, and `:batch/applied` event with no new plumbing. The only `alpha.clj` edit is the `apply!` docstring, which must state the two-op grammar, the exact public transition shape, submitted refs versus durable row ids, the replacement-upsert pre-image, and the equal ordered result/hook/event vectors with no `:edge` alias.
