...
- **Decision:** `:remove` names identity `(from, to, type)` alone and deletes only that row. A clean post-lock lookup that finds no matching row — wrong direction and wrong relation type included — fails loudly with structured `ex-data` naming the refs, resolved durable ids, and relation, and rolls the whole batch back.
...
- **Rationale:** Ordered execution is what makes a legal program commit and an illegal order roll back: a `serves` swap or a DAG reversal only succeeds when the remove precedes the upsert (`PROP-Xer-001.T1`, `PROP-Xer-001.T2`). Replacement is caller-composed from the ordered pair, not a new op.
...
- **Decision:** Every edge outcome is the before/after transition of `CC2`, replacing the upsert-only `:edge` outcome with no compatibility alias. The result `:edges`, the `:batch/apply-before-commit` hook's `:batch/edge-ops`, and the `:batch/applied` event's `:batch/edges` carry the same ordered vector by value, including before-images and after-images.
