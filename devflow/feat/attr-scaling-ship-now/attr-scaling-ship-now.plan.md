...
- **MED — attr-get guard ex-data pinned.** `CC6` (strand-model) and `CC7` (daemon) now commit the guard's `ex-info` to carry the offending key + the `show <id>` recovery path, matching the `require-valid-relation-name!` precedent. Plan `AA6`/`PH2`/`AA7` updated.
