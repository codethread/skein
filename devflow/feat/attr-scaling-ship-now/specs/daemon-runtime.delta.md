...
- **ASSN-DELTA-003.CC5:** The query compiler emits a literal JSON path for a predicate over a declared key and the existing bound-parameter form for an undeclared key (`ASSN-DELTA-001.CC12`/`CC13`). Compilation consults the durable registry as the source of truth and re-validates the key against the same `::specs/indexed-attr-key` spec before literal emission (`ASSN-DELTA-001.CC12`), so declaration and compilation cannot disagree on what is safe to embed; result semantics are identical for both forms.
- **ASSN-DELTA-003.CC6:** Weaver API return-value normalization (SPEC-004.C17) is unchanged for the trusted in-process path: `skein.api.weaver.alpha` reads (`show`, `list`, `ready`, `strands-by-ids`, `subgraph`, and the query-execution ops as consumed **in process**) decode attributes to full-fidelity Clojure maps. The lean tier is a CLI/agent read-surface transform layered above these ops (`ASSN-DELTA-002.CC4`), so the trusted path a spool calls never applies it.
- **ASSN-DELTA-003.CC7:** The lean-tier omission descriptor is the shared `::specs/omitted-attribute-descriptor` contract (`ASSN-DELTA-001.CC3`). The canonical trusted spool reader `attr-get` fails loudly when handed a value conforming to that spec where a raw value is expected (`ASSN-DELTA-001.CC6`); its `ex-info` pins the ex-data contract — the offending attribute key plus the full-fidelity `show <id>` recovery path — a newly owned contract in the fail-loud spirit of `require-valid-relation-name!` (deliberately richer than that precedent's bare `{:relation relation}`), so a misrouted lean-projected strand surfaces immediately with enough context to recover rather than delivering a descriptor to a payload consumer (TEN-003).
