...
Every headless outcome that observes the process exit records that code on `agent-run/exit-code` — the `done` close, the blank-result-exit-0 failure, and the non-zero-exit failure all stamp it. Failures with no process exit code to report (interactive session death, launch exceptions, a killed run) deliberately omit the attribute; its presence means "a headless process reported this code", not "the run failed".
...
| `agent-run/error` | Failure detail when phase is `failed` or `exhausted`. A headless exit 0 with an empty result reads `harness exited 0 with an empty result`. |
| `agent-run/exit-code` | Process exit code of a headless run, recorded on every outcome that observed one: `done`, blank-result-exit-0 failure, and non-zero-exit failure. Absent when no process reported a code (interactive death, launch exception, kill). |
