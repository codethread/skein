...
| `(fanout-ceiling)` | Return the workspace headless fan-out ceiling `W` the `claim!` window enforces. Defaults to `4`. |
| `(set-fanout-ceiling! n)` | Set the workspace ceiling `W` on concurrent headless runs (positive integer; invalid values fail loudly). Trusted-config setter, like `set-preamble-extension!` and the batteries read cap; reload-tolerant through the engine's versioned state. |
