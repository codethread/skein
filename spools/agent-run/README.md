...
| `(scan!)` | Spawn every ready pending run the fan-out window admits, unless already claimed in this weaver lifetime. A run the window has no slot for stays pending with no attribute until a completion frees a slot and re-fires the scan. Usually called by events and install/reconcile. |
...
| `(set-fanout-ceiling! n)` | Set the workspace ceiling `W` on concurrent headless runs (positive integer; invalid values fail loudly). Trusted-config setter, like `set-preamble-extension!` and the batteries read cap; reload-tolerant through the engine's versioned state. The window reads the ceiling internally — there is no public getter. |
...
A concurrency window layers back-pressure over readiness at run start without becoming a second scheduler. `claim!` admits a ready headless run only while fewer than the workspace ceiling `W` headless runs are already in flight; `W` defaults to `4` and trusted config overrides it with `set-fanout-ceiling!` (workspace-wide, alongside the batteries read cap). A run stamped with a fan-out `agent-run/fanout-group` and a requested `agent-run/fanout-cap` K tightens further to `min(W, K)` within that group, never exceeding the workspace width. Interactive sessions are exempt and consume no slot. A run the window refuses stays pending with no new attribute — a later completion re-fires the scan and admits the next — so this is pure admission back-pressure: no queue, no new graph edges, and readiness stays the sole scheduler.
...
| `agent-run/fanout-group` | Optional fan-out group id shared by the runs of one delegation fan-out; the `claim!` window caps headless runs in the group to `min(W, cap)`. |
