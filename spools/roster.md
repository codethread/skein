...
| `(await-quiet! runtime opts)` | **C5** — Block until the selected scope has no active non-stale entries. Returns `{:reason :quiet\|:stale\|:timeout :entries [...]}`. Scopes on `:feature`, `:branch`, `:worktree`; non-negative `:timeout-secs` (default thirty minutes), `:stale-after-ms` (default fifteen minutes), and positive `:poll-ms` (default fifty) tune the wait. The wait uses the runtime Clock, and `:stale` short-circuits ahead of `:quiet`. |
