...
| `(await-quiet! runtime opts)` | **C5** — Block until the selected scope has no active non-stale entries. Returns `{:reason :quiet\|:stale\|:timeout :entries [...]}`. Scopes on `:feature`, `:branch`, `:worktree`; `:timeout-ms` (default thirty minutes) and `:stale-after-ms` (default fifteen minutes) tune the wait. `:stale` short-circuits ahead of `:quiet`. |
