# Table of contents
-  [`skein.spools.cron`](#skein.spools.cron)  - Userland recurrence layer over the weaver's durable scheduler wake primitive.
    -  [`await-idle!`](#skein.spools.cron/await-idle!) - Block until every offloaded cron job on <code>runtime</code> has finished, then return <code>runtime</code>.
    -  [`deregister!`](#skein.spools.cron/deregister!) - Cancel a cron job's pending wake and remove it from <code>runtime</code>.
...
``` clojure
(await-idle! runtime)
(await-idle! runtime {:keys [timeout-ms]})
```
Function.
