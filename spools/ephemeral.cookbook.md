...
**Composition.** Create each scratch strand with `ephemeral/add` under the task root, so it carries the `ephemeral/entry "true"` attribute and a `parent-of` edge from the root. When the task is done, call `ephemeral/burn-all!` once to clear them.
...
(ephemeral/add (:id task) "Scratch: malformed-line cases" {:owner "agent"})
(ephemeral/add (:id task) "Scratch: upstream API returns 204 on empty" {:owner "agent"})
...
- **The strand is ordinary; only the attribute is special.** Nothing in the
  ...
  no lifecycle hook. `ephemeral/burn-all!` is a call *you* make at the boundary
  of the work. That is the honest contract, and it is why the whole spool
  composes just the documented weaver and graph helpers (contract
  [§1](./ephemeral.md#1-overview)).
...
- **One burn call, not N deletes.** Because the attribute marks the whole class,
  you clear the round's scratch with a single `ephemeral/burn-all!` instead of
  tracking and closing each id by hand.

Honest source: the `ephemeral/add` / `ephemeral/ids` / `ephemeral/burn-all!` round-trip verified against a live weaver, mirroring the ephemeral case in [`test/skein/spools_test.clj`](../test/skein/spools_test.clj) (`ephemeral-spool-composes-public-helper-surfaces`).
...
**Composition.** `ephemeral/burn-all!` is workspace-wide by design, so scope it yourself: AND the reusable `ephemeral/query` together with a `parent-of` edge filter for the parent you're finishing, then burn exactly those ids.
...
(ephemeral/add (:id task-a) "A scratch")
(ephemeral/add (:id task-b) "B scratch")
...
(def a-scratch
  [:and ephemeral/query [:edge/in "parent-of" [:= :id (:id task-a)]]])
...
- **You reuse the same burn primitive.** Scoping changes only *which ids* you
  compute; `burn-by-ids!` does the same closing work `ephemeral/burn-all!` does under
...
**Composition.** The spool is a thin helper over an attribute convention, so the shipped `strand` ops reach the same shape directly: `add` with `--attr ephemeral/entry=true`, an `update --edge parent-of:<id>` to home it, and a **named query** registered once so `list` and `burn` can find the class.

```sh
# create a scratch strand carrying the convention, and home it under the task
child=$(strand add "Scratch: failing case" --attr ephemeral/entry=true --attr owner=agent)
...
(require '[skein.spools.ephemeral :as ephemeral]
         '[skein.repl :as repl])
(repl/defquery! 'ephemeral ephemeral/query)
```
...
- **There is no `strand ephemeral` op, and there doesn't need to be.** The value
  is the attribute, not a command — `--attr ephemeral/entry=true` on an ordinary `add`
...
- **REPL and CLI are the same convention, not two systems.** A strand created
  with `ephemeral/add` shows up in the CLI's named-query list, and one created with
  `--attr ephemeral/entry=true` is burned by `ephemeral/burn-all!`. Pick whichever surface
  you're already in.
...
**Composition.** Don't. A durable outcome is an ordinary strand (or a note on the strand it belongs to). Reserve `ephemeral/add` for state that is worthless once the current run ends.

```clojure
;; WRONG: a decision you'll want tomorrow, marked to be burned tonight
(ephemeral/add (:id task) "Decided: wide table, no joins")
...
- **The test is lifespan, not size.** "Small" and "quick" are not reasons to mark
  ...
  think is ephemeral.
- **`ephemeral/burn-all!` is indiscriminate.** It clears the whole active ephemeral
