...
> This is the **contract** doc: the checkset model, check forms, watch-mode
...
> (how/why you grow and enforce a checkset) — and
...
It is deliberately userland convention, not an engine schema. Checksets are weaver-lifetime data, checks are ordinary maps, and violations are returned as data.
Invalid checkset specs fail loudly at registration time.
...
(selvage/check (:id run))
;; => [{:strand-id "..." :checkset :agent-run :attr "agent-run/phase" ...}]
...
| Fn | Behavior |
|---|---|
| `(register-checkset! name spec)` | Register or replace one weaver-lifetime checkset. Unknown spec/check keys fail with allowed sets in `ex-data`. |
| `(checksets)` | Return registered checkset metadata, sorted deterministically. |
| `(unregister-checkset! name)` | Unregister a checkset; absent names fail loudly. |
...
| `{:attr s :type t}` | If attribute `s` is present, its value must match `t`, one of `:string`, `:number`, `:boolean`, `:map`, or `:int-string`. |
