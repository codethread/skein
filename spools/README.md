...
| `skein.spools.dresser` | *(none approved in this repo)* | [dresser.md](https://github.com/codethread/dresser.spool/blob/main/dresser.md) | — | Convention convergence: versioned per-aspect setup/verify workflows (`spool-repo` and `skein-dir` flavours) driven from an operator world against a target repo path, verified by `:shell` gates, receipted in the target at `.skein/conventions.edn`. |
...
`skein.spools.dresser` is the third external spool ([`codethread/dresser.spool`](https://github.com/codethread/dresser.spool)): it bootstraps and maintains opinionated repo conventions as per-aspect convergence workflows, in two flavours — scaffolding a shared-spool repo shaped like devflow.spool/kanban.spool, and installing a self-contained `.skein/` workspace into any host repo. It runs in whichever workspace approves and activates it (the target repo needs no weaver), so this repo approves no coordinate for it; its README carries the full dependency/activation recipe.
