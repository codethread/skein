...
| `skein.spools.brief` | [brief.md](./brief.md) | [brief.api.md](./brief.api.md) · [cookbook](./brief.cookbook.md) | Brief/guide primitive: per-run contracts (mission/deliverable/scope/budgets) and durable authoring guides as closed-key data, composed from named clause blocks and rendered through one shared `brief->prompt` seam, plus `brief/owns`/`brief/budgets` projection attrs, an `overlapping-owns` collision detector, the `guide/key` step convention, and a fetch-only `brief` op. |
