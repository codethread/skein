...
A coordinator recovers such a gate by clearing its `treadle/run` attribute (optionally re-writing `shuttle/prompt`). A CLI clearing idiom is `--attr treadle/run=` (empty string, since there is no attribute-delete flag): the next scan finds no live run for the gate and spawns a fresh delegation — a run stamped `treadle/gate` and linked by a `delegates` edge, which the treadle then delivers. Stamping the fresh run repoints the dead run's provenance with `treadle/superseded-by`, so the gate's stale `delegates` edge stops surfacing it in `stalled-gates` once a healthy replacement is in flight. Clearing the stamp, not retrying, is what triggers that fresh delegation. The same empty-string rule applies to `treadle/error`: `--attr treadle/error=` clears a spawn-side error for the next scan.
