(ns skein.api.return-shape.internal
  "Implementation plumbing for `skein.api.return-shape.alpha` (SPEC-003.C19a).

  Everything here may change freely; the promised surface and its docstrings
  live in the sibling alpha namespace. Thrown ex-data keeps the published
  alpha-qualified marker `:skein.api.return-shape.alpha/error` even though
  the code lives here — the key is contract, the location is not."
...
(defn validate-shape!
  ...
  (cond
    ...
    :else
    (fail! :invalid-shape
           (str "Return shape must be a supported scalar, nullable scalar,"
                " map, or collection declaration")
           {:path path :value shape})))
