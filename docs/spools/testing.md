...
- **`:load :image` needs the namespace loaded and a contribution entry point.** Test namespaces normally require the spool they exercise, so the image is loaded — and its `spool` var resolvable — by the time the fixture runs. During Phase A only, an explicit legacy `:contribute` may supply the entry point when the loaded namespace has no public `spool` var; an unloaded namespace, or a loaded one with neither source, fails loudly.
