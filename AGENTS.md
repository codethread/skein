...
- [Reed Shell Gate Executor](./spools/reed.md) (classpath-shipped spool: fulfils `:shell` workflow gates by running the gate's `shell/argv` command directly, closing the gate on a zero exit and stamping a loud `shell/error` otherwise)
